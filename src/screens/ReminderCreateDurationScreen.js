...
export default function ReminderCreateDurationScreen({ navigation, route }) {
  const draft = route?.params?.draft ?? { title: "", date: null, time: null, durationSec: null };

  const [value, setValue] = useState(
    draft.durationSec != null ? String(draft.durationSec) : "10"
  );
  ...
  const goNext = () => {
    ...
    navigation.navigate("ReminderCreateSuccess", {
      draft: {
        ...draft,
  ...
  return (
    <View style={styles.container}>
      <View style={styles.center}>
        <Text style={styles.heading}>Klingel Dauer</Text>
