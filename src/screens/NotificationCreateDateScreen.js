...
export default function NotificationCreateDateScreen({ navigation, route }) {
  const [showPicker, setShowPicker] = useState(false);

  const draft = route?.params?.draft ?? { text: "", date: "", time: "" };
  ...
  const goNext = () => {
    ...
    navigation.navigate("NotificationCreateTime", {
      draft: {
        ...draft,
        date: value,
      },
  ...
  return (
    <View style={styles.container}>
      <View style={styles.center}>
        <Text style={styles.heading}>Notification Datum</Text>
